...
def pwd():
    ...
    if form.validate_on_submit():
        ...
        admin.pwd = generate_password_hash(data['new_pwd'], method='pbkdf2:sha256', salt_length=8)
...
def movie_add():
    ...
    if form.validate_on_submit():
        data = form.data
        file_url = secure_filename(form.url.data.filename)  # 选择文件的名字
        file_logo = secure_filename(form.logo.data.filename)
        if not os.path.exists(app.config["UP_DIR"]):
            os.makedirs(app.config["UP_DIR"])
            os.chmod(app.config["UP_DIR"], "rw")  # 对这个目录授权可读可写
...
def comments_list(page=None):
    ...
    return render_template("admin/comments_list.html", page_data=page_data)
